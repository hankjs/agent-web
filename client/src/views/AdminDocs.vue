...
import { listRequirementDocs, type RequirementDoc, type Paginated } from "../api/admin";
...
async function fetchDocs() {
  ...
  try {
    const res = await listRequirementDocs({
...
const statusTabs = ["", "draft", "confirmed", "completed"];
...
<template>
  <div class="changes-view">
    <header class="view-header">
      <span class="view-title">需求文档</span>
    ...
    <div class="view-body">
      ...
      <table v-else-if="docs.length" class="changes-table">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="doc in docs" :key="doc.id">
            ...
            <td class="col-status">
              <span class="status-badge" :class="doc.status">{{ statusLabels[doc.status] || doc.status }}</span>
            </td>
            <td class="col-ver">v{{ doc.version }}</td>
            <td class="col-time">{{ relativeTime(doc.updated_at) }}</td>
          </tr>
        </tbody>
      </table>
      <p v-else class="empty">暂无需求文档</p>
...
.search-input { padding: var(--space-1) var(--space-2); border: 1px solid var(--color-border-subtle); border-radius: var(--radius-sm); font-size: 11px; background: var(--color-surface-1); color: var(--color-text-primary); width: 140px; }
...
.col-ver { width: 50px; color: var(--color-text-muted); }
...
.status-badge.draft { color: var(--color-text-muted); background: var(--color-surface-2); }
.status-badge.confirmed { color: var(--color-info); background: var(--color-info-surface); }
